...
statusWidget cmd = do
    ...
    commandRunnerNew 0 (dataDir ++ "/status/" ++ cmd) [] $ T.pack ""
...
taffybarConfig = defaultSimpleTaffyConfig
  ...
  where
    ...
    clock         = textClockNew Nothing ((colorize "#383838" "#282828" "\xe0b2") ++ (colorize "#383838" "#383838" "\x2588") ++ (colorize "#d8d8d8" "#383838" "%a %b %_d %r ")) 1
    statusWidgets = map statusWidget ["vpn", "screensaver", "backlight", "audio", "network", "power", "load"]

main :: IO ()
main = do
    let monitorTypes = ["vpn", "screensaver", "backlight", "audio", "network", "power"]
    dataDir <- getXdgDirectory XdgConfig "taffybar"
    monitorProcs <- mapM (\base -> spawnProcess (dataDir ++ "/status/" ++ base) ["monitor"]) monitorTypes
    installHandler sigTERM (Catch (mapM_ terminateProcess monitorProcs >> Gtk.mainQuit)) Nothing
